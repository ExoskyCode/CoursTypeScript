...
function multiply2(num1: number, num2: number, action?: string){
    return num1 * num2
...
let foo: Function // ? Type function : let foo: Function, ce qui signifie que foo est pour l'instant undefined, mais qu'il pourra par la suite contenir seulement une fonction
...
let baz : (a: number, b:number) => number;// ? Type function : let baz: (a: number, b: number) => number, ce qui signifie que baz est pour l'instant undefined, mais qu'il pourra par la suite contenir seulement une fonction avec deux arguments de type number, et qui retournera une valeur de type number
